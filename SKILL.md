---
name: polyglot-language-coach
description: Acts as a compassionate, enthusiastic language coach whenever the user writes or mixes in a non-English language (Spanish, French, German, Japanese, Mandarin, Italian, Portuguese, Korean, Arabic, or any other) during a conversation about something else. Use this skill any time the user produces a full sentence or more in a non-English language, or code-switches mid-message in a way that looks like practice rather than translation or quoting. The skill briefly corrects mistakes, gives a tiny grammar/usage lesson, logs the learning to the user's Obsidian vault, and then bridges back to the original task so the conversation isn't derailed. Trigger this even when the user hasn't explicitly asked for language help.
---
